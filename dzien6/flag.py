...
def update_the_picture():
    updated_picture = ImageTk.PhotoImage(Image.open(new_img()).resize((600, 338)), Image.Resampling.BICUBIC)
...
def check(*args):
    ...
    nazwa_panstwa.place(x = 400, y = 400)
    if (panstwo == nazwa):
...
img = ImageTk.PhotoImage(Image.open(new_img()).resize((600, 338)), Image.Resampling.BICUBIC)
w = Label(window, image = img)
w.place(x = 10, y = 10)
b = Entry(window, width = 40, font = ("Calibri", 12), justify = "center")
b.place(x = 20, y = 360)
c = Label(window, text = "Wybór państwa ->", bg = "#121212", fg = "white")
c.place(x = 365, y = 360)
wypisane_kraje = Label(window, text = "Ostatnie państwa: ", bg = "#121212", fg="white")
wypisane_kraje.place(x = 655, y = 30)
OPTIONS = [
...
variable = StringVar()
variable.set("")
wysuwana_lista = Combobox(window, textvariable = variable, values = OPTIONS)
wysuwana_lista.place(x = 470, y = 360)
wysuwana_lista.bind('<<ComboboxSelected>>', check)
wybrane_panstwo = wysuwana_lista.get()
zaslona = ImageTk.PhotoImage(Image.open("black.jpg").resize((200, 169)), Image.Resampling.BICUBIC)
